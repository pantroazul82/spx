...
public class clsSeguridad
{
    ...
    public List<string> verCorreos(int codPermiso) 
    {
        SPXSeguridad.data.cls.clsTusuario u = new SPXSeguridad.data.cls.clsTusuario();
        SPXSeguridad.data.dsSPXSeguridad ds =new SPXSeguridad.data.dsSPXSeguridad();
        u.FillByPermiso(ds.usuario, codPermiso);

        List<string> lista = new List<string>();
        for (int k = 0; k < ds.usuario.Count; k++)
        {
            if (ds.usuario[k].IscorreoNull() || ds.usuario[k].correo.Trim() == string.Empty || ds.usuario[k].correo.IndexOf('@') < 0) continue;
            lista.Add( ds.usuario[k].correo);
        }
        return lista;
    }
    ...
    {
        string mensaje="";
        return enviarEmail(asunto, cuerpo,  direccionDestino,  out mensaje);
    }

    public bool enviarEmail(string asunto, string cuerpo, string direccionDestino, out string mensaje)
    {
    ...
    {
        System.Configuration.AppSettingsReader ar = new System.Configuration.AppSettingsReader();
        return ar.GetValue("ipsitio", typeof(string)).ToString();
    }

    public bool enviarEmail(string asunto, string cuerpo, string direccionDestino, List<string> adjuntos, out string mensaje)
    {
        System.Configuration.AppSettingsReader ar = new System.Configuration.AppSettingsReader();
        string cuentaFrom = ar.GetValue("cuentaFrom", typeof(string)).ToString();
        string nombreCuentaFrom = ar.GetValue("nombreCuentaFrom", typeof(string)).ToString();
 ...
 try
        ...
        DateTime fechaInicio,DateTime fechaFin,string rutaTemp,string direcionDestino)
    {
        ...
        l.Add( file );
        string msg = "";
        enviarEmail(asunto, observaciones, direcionDestino,l,out msg);
    }
